...
function buildcopy() {
    return src([
        'src/js/scripts.min.js', '!src/js/*.json', 'src/styles/styles.min.css', 'src/fonts/**/*',

        'src/*.*', '!src/*.html', '!src/*.php',

        'src/img/**/*.*', '!src/img/src/**/*', '!src/img/dist/**/old/**.*',
...
function startwatch() {
    ...
    watch('src/img/src/**/*.{jpg,jpeg,png,webp,svg,gif}', { usePolling: true }, img)
    watch(`src/**/*.{php,html,htm,txt,json,md,woff2}`, { usePolling: true }).on('change', browserSync.reload)
}
...
exports.build   = series(cleandist, scripts, styles, cleanimg, img, buildcopy, removeComments)
